...
BinaryHeap.prototype.insert = function (value) {
  let nodeIdx = this._heap.length;
  this._heap.push(value);
  let parentIdx = Math.floor((nodeIdx - 1) / 2);
  while (parentIdx >= 0 && this._compare(this._heap[nodeIdx], this._heap[parentIdx])) {
    let temp = this._heap[nodeIdx];
    this._heap[nodeIdx] = this._heap[parentIdx];
    this._heap[parentIdx] = temp;
    nodeIdx = parentIdx;
...
BinaryHeap.prototype.removeRoot = function () {
  ...
  let nodeIdx = 0;
  let childIdx = [nodeIdx * 2 + 1, nodeIdx * 2 + 2];
  while (this._compare(this._heap[childIdx[0]], this._heap[nodeIdx])
    || this._compare(this._heap[childIdx[1]], this._heap[nodeIdx])) {
      let temp = this._heap[nodeIdx];
      if (!this._heap[childIdx[1]] || this._compare(this._heap[childIdx[0]], this._heap[childIdx[1]])) {
        this._heap[nodeIdx] = this._heap[childIdx[0]];
        this._heap[childIdx[0]] = temp;
        nodeIdx = childIdx[0];
      } else {
        this._heap[nodeIdx] = this._heap[childIdx[1]];
        this._heap[childIdx[1]] = temp;
        nodeIdx = childIdx[1];
      }
      childIdx = [nodeIdx * 2 + 1, nodeIdx * 2 + 2];
  }
  return root;
}

const minHeap = new BinaryHeap(function (i, j) { return i > j });
